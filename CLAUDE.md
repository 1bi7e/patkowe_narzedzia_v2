...
- **Day settlement locks the day — with one controlled way back.** Once a day is settled (`day_settlements`), its payment entries lock; enforced in the database (triggers), not just in the UI. Clients cannot mutate settlements directly — the only undo path is RPC `cofnij_rozliczenie` (SECURITY DEFINER, transaction flag `app.korekta_rozliczenia`; see `supabase/migrations/20260718120000_cofanie_rozliczen.sql`), surfaced in the UI behind „…" on the settled-day card in Historia (confirmation sheet). A settlement also stores `gotowka_dla_agaty_grosze` (cash Patrycja owes Agata = Agata's cards − her card assignments) with a „przekazana" checkmark toggled via RPC `oznacz_gotowke_oddana`.
- **The „Rozliczenia" screen shows *all unsettled days*, not just today** — grouped by day. The read model `grupujNierozliczone` returns days oldest-first (its test asserts this), but the **screen displays today's day on top and older days below** (reversed only in the view — [src/screens/RozliczeniaScreen.tsx](src/screens/RozliczeniaScreen.tsx)), and payments within a day newest-first. The header is just the „Rozliczenia" wordmark (no date) + two per-stylist summary tiles (karta/gotówka summed across all unsettled days); the date lives on each day-group header. A payment is unsettled ⟺ `locked = false` (settlement locks a day's entries and a settled day rejects new ones), so the read model needs no schema change — see [src/lib/nierozliczone.ts](src/lib/nierozliczone.ts) + [src/lib/useNierozliczone.ts](src/lib/useNierozliczone.ts). One settlement action may cover **several selected unsettled days atomically**: with ≥2 unsettled days the screen shows per-day checkboxes (default unchecked, to protect the in-progress day); the settle screen ([src/screens/RozliczScreen.tsx](src/screens/RozliczScreen.tsx)) calls RPC `rozlicz_dni` (one `day_settlements` row per day, all-or-nothing; `rozlicz_dzien` delegates to it). ⚠️ This **deliberately extends** the spec/design, which describe a single-day „Dziś" view — do not "fix" it back to today-only. Screen files: [src/screens/RozliczeniaScreen.tsx](src/screens/RozliczeniaScreen.tsx) (was `DzisScreen`), tab „Rozliczenia" in [src/components/BottomNav.tsx](src/components/BottomNav.tsx). Card assignment to a cost at settlement (Sposób 2) is now implemented: the settle screen shows an "Przypisz karty na koszt" section (uncovered costs, capped at Agata's card takings for the selected days) and passes the collected `PrzypisanieKart[]` to `rozlicz_dni`. The home screen is also the **task surface**: settlements with unhanded cash (`gotowka_dla_agaty_grosze > 0` and not `gotowka_oddana`) show gold „Do oddania Agacie" task cards above the summary tiles, with a one-tap „Oznacz jako przekazaną" (cards disappear once checked; the state stays visible/undoable on the settlement card in Historia).
- **The „Finanse" tab has three sub-tabs** — Podsumowanie / Koszty / Historia ([src/screens/FinanseScreen.tsx](src/screens/FinanseScreen.tsx)); there is **no „Rozliczone" sub-tab** (deliberate IA: tasks live on home, records in Finanse — don't reintroduce it). The period (month / custom range) is picked from the tappable header overline (bottom sheet `SheetOkresu`) and applies to Podsumowanie + Historia only. Historia is the archive: a unified feed of payments + costs grouped by day, and a settled day gets a settlement card ([src/components/KartaRozliczenia.tsx](src/components/KartaRozliczenia.tsx)) at the top of its group — cash-handover checkbox plus undo behind „…". `useRozliczone` lives permanently in [src/screens/AppShell.tsx](src/screens/AppShell.tsx) and feeds both the home task cards and Historia; shared correction actions (offline guard, double-tap lock, „already undone" race) sit in [src/lib/useAkcjeRozliczenia.ts](src/lib/useAkcjeRozliczenia.ts).
